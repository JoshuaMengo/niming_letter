<template>
  <div class="FeedbackRecord">
    ...
    <div class="content" v-if="list">
      <div class="list_item" @click="enterDetail(item.fid)" v-for="(item, index) in list" :key="index">
        <div>
          <div>用户ID：{{item.uid}}</div>
          <div>{{item.create_time}} {{item.is_handle?'已处理':'未处理'}}</div>
        </div>
        <div>></div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    queryList(){
      ...
      queryFeedback(session).then(res=>{
